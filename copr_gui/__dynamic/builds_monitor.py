...
class ContextBuildsMenu(ContextCommon):
    def __init__(self, parent, obj):
        super().__init__(parent, [
            'Show result', "Show chroots", "Clone", "Stop", "Drop"], obj)
...
class BuildsMonitor(MonitorCommon):
    ...
    def refresh_object(self, obj):
        ...
        def build(obj):
            ...
            self.add([build])
        uistatusbar.execute_data_with_progress([obj], build, 'transferring', f'Refresh build with id {obj["id"]}')
    ...
    def run_add(self, settings):
        uistatusbar.execute_data_with_progress([settings], self.add_build, 'transferring', 'Add build')
                ...
                def build():
                    for i in items:
                        try:
                            ...
                            yield None
                uistatusbar.execute_with_progress(build(), count,
                    'transferring', 'Build package')
...
'Build Id', 'Package Name', 'Package Version', 
{'name':'DateTime of Start','type':'date'}, 
{'name':'DateTime of End','type':'date'}, 'Status'], **kwargs)
        none = D('')
        zero = D(0)
        src = 'source_package'
        id = S(C(I('id'), zero), zero) 
        name = S(C(I(src, 'name'), none), none)
        ver = S(C(I(src, 'version'), none), none)
        ston = T(S(C(I('started_on'), zero), zero))
        enon = T(S(C(I('ended_on'), zero), zero))
        state = S(C(I('state'), none), none)
...
class BuildChrootsMonitor(BuildsMonitor):
    ...
    def refresh_object(self, obj):
        client = self.client
        def build(obj):
            build = client.build_chroot_proxy.get(self.build['id'], obj['name'])
            self.add([build])
        uistatusbar.execute_data_with_progress([obj], build, 'transferring', f'Refresh build chroot with name {obj["name"]}')
    ...
    def __init__(self, parent, config_args, filter_args=None, **kwargs):
        MonitorCommon.__init__(self, parent, [
            {'id':'add','name': 'Clone'}, 'Stop'], 
            ['Chroot Name', {'name':'DateTime of Start','type':'date'}, 
{'name':'DateTime of End','type':'date'},  'Status'], 
