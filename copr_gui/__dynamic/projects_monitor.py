...
class ContextProjectsMenu(ContextCommon):
    def __init__(self, parent, obj):
        super().__init__(parent,
            ['Show builds', 'Show packages', 'Show chroots', 'Edit', 'Drop'], obj)
    ...
    def on_show_option(self, event, monitor):
        ...
        owner = obj['ownername']
        parent = self.parent
        run_monitor(monitor, None, parent.client,
            owner, project, filter_args={})
...
class ProjectsMonitor(MonitorCommon):
    ...
    def refresh_object(self, obj):
        ...
        def build(obj):
            ...
            self.add([build])
        uistatusbar.execute_data_with_progress([obj], build, 'transferring', f'Refresh project with name {obj["name"]}')
    ...
    def run_add(self, settings):
        uistatusbar.execute_data_with_progress([settings],
            self.add_project, 'transferring', 'Add/edit project')
...
'Project Name', 'Project Description'], **kwargs)
        none = D('')
        name = S(C(I('name'), none), none)
        description = S(C(I('description'), none), none)
